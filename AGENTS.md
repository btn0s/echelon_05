...
- `StealthSimulationSubsystem` light sampling is renderer-independent (direct line-traced scene-light intensity); `SceneLightAmbientExposure=0.06` is a hardcoded floor that must track active PPV exposure/indirect overrides using priority/blend resolution to stay in sync with what the player sees.
- `L_StealthLightLab` uses a roofed fortress lighting setup: `PPV_StealthNoir`, DirectionalLight intensity 3 with no indirect/atmospheric contribution, `SkyLight_AmbientFloor` at 0.04, and `FixturePool_A/B` interior work-light pools. `SunSlit_A/B` are obsolete with the roof and should not return as the lighting grammar.
...
- In sibling project `EchelonStealthOp`, `Content/TSO/Character/UE5Char/Meshes/SKM_Manny.uasset` and `SKM_Quinn.uasset` are custom stealth character meshes with headgear despite standard mannequin names; treat them as candidate player stealth meshes.
