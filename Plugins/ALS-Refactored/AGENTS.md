...
**Linear TH-505:** treat locomotion investigation and navigation as **this plugin subtree only** — packaged locomotion lives under **`Content/`** as binary **`*.uasset` / `*.umap`** (layout and filenames in git; graph internals need Unreal Editor).
...
| Packaged Unreal assets | [`Content/AGENTS.md`](Content/AGENTS.md) — **`*.uasset` / `*.umap`** only under **`Plugins/ALS-Refactored/Content/`** (binary; folder/layout in git, not graph source) |

## Agent rules of thumb

1. **Do not invent AnimBP / BT / uasset behavior** — cite `SYSTEM_MAP.md` gap register when assets are missing from the tree.
